...
function initGrid () 
{
  for (let counter = 0; counter < (size * size); counter++) {
     const col = counter % size;
     const row = (counter - col) / size;
     turnOff(row, col);
  }
}
...
{
  grid[row * size + col] = grid[row * size + col] === 1 ? 0 : 1;
}

function turnOn (row, col)
{
    grid[row * size + col] = 1;
}

function turnOff (row, col)
{
  grid[row * size + col] = 0;
}

function toggleSquare (start1, start2, end1, end2)
{
  for (let counter1 = start1; counter1 <= end1; counter1++) {
    for (let counter2 = start2; counter2 <= end2; counter2++) {
      toggle(counter1, counter2);
    }
  }
}

function turnOnSquare (start1, start2, end1, end2)
{
  for (let counter1 = start1; counter1 <= end1; counter1++) {
    for (let counter2 = start2; counter2 <= end2; counter2++) {
      turnOn(counter1, counter2);
    }
  }
}

function turnOffSquare (start1, start2, end1, end2)
{
  for (let counter1 = start1; counter1 <= end1; counter1++) {
    for (let counter2 = start2; counter2 <= end2; counter2++) {
      turnOff(counter1, counter2);
    }
  }
}
...
{
  return grid.filter(v => v === value).length;
}
